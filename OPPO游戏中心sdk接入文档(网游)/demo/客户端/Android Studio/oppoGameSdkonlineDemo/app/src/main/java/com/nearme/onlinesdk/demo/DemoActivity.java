...
public class DemoActivity extends Activity {
	...
	private void sendRoleInfo() {
		...
		new MainThreadHandler().postDelayed(new Runnable() {
			@Override
			public void run() {

				HashMap<String, String> accountId = new HashMap<>();
				accountId.put("key", "accountId");
				accountId.put("value", "g234678902345671");

//上报角色ID
				HashMap<String, String> roleId= new HashMap<>();
				roleId.put("key", "roleId");
				roleId.put("value", "234578");
				roleId.put("updateTime", "2023-08-30 08:37:55");
				roleId.put("desc", "自定义的角色ID的描述信息");

//上报游戏区服信息
				HashMap<String, String> gameZoneInfo= new HashMap<>();
				gameZoneInfo.put("key", "gameZoneInfo");
				gameZoneInfo.put("value", "9");
				gameZoneInfo.put("desc", "裁决之地");
				gameZoneInfo.put("unId", "G-123123");//该区服在游戏方的唯一ID
				gameZoneInfo.put("createTime", "2023-08-30 09:37:55");

				List<Map<String, String>> reportData = new ArrayList();

				reportData.add(accountId);
				reportData.add(roleId);
				reportData.add(gameZoneInfo);
				GameCenterSDK.getInstance().doSendGameData(reportData,
						new ApiCallback() {
							@Override
							public void onSuccess(String resultMsg) {
								Toast.makeText(DemoActivity.this, "doSendGameData success",
										Toast.LENGTH_LONG).show();
							}

							@Override
							public void onFailure(String resultMsg, int resultCode) {
								Toast.makeText(DemoActivity.this, "doSendGameData fail",
										Toast.LENGTH_LONG).show();
							}
						});
			}
